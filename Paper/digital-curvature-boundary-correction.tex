...
We evaluate the quality of a boundary by evaluating the elastica energy along it. In continuous terms, with $\alpha,\beta > 0$, the elastica energy along the boundary of a region $X$ is defined as:
...
where $\mathcal{N}_4(x)$ denotes the $4$-adjacent neighbot set of $x$. The following optimization regions will be important in our process.
...
We proceed by minimizing the squared curvature energy along $A$ with respect to the optimization region $O$. 

\begin{align}			
	Y^{\star} = \argmin_{Y \in \{0,1\}^{|O|}} \sum_{x \in A}{\hat{\kappa}_{r}^2(x)}.
	\label{eq:corner-detector-opt-not-exp}
\end{align}

Note that we are minimizing second term of equation \eqref{eq:ideal_optimization_energy}.
...
Figure \ref{fig:butterfly_results} display some results.
	...
	\begin{figure}[!ht]
		\center
		\subfloat[\label{fig:butterfly_base}]{%
		\includegraphics[scale=0.5]{images/butterfly/baseImg.jpg}
		}%
		\hspace{15pt}
		\subfloat[\label{fig:butterfly_corrected}]{%
		\includegraphics[scale=0.5]{images/butterfly/c1d1-it5.jpg}
		}%

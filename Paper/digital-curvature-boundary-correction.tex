...
We have some directions for future work. First, optimize the code and evaluate a runtime analysis to compare with competitor methods. We also pretend to reformulate the model in \cite{schoenemann09} using the digital estimator $\hat{\kappa_r}$. We believe that we can recover better results and in a lower running time.
	...
	\begin{figure}[!ht]
		...
		\includegraphics[scale=0.5]{images/butterfly/initial.jpg}
		}
		
		\subfloat[\label{fig:butterfly_gc}]{%
		\includegraphics[scale=0.25]{images/butterfly/gc-marks.jpg}
		}%
		\subfloat[\label{fig:butterfly_cor}]{%
		\includegraphics[scale=0.25]{images/butterfly/cor.jpg}
		}
		
		\subfloat[\label{fig:butterfly_gc_m1}]{%
		\includegraphics[scale=0.5]{images/butterfly/gc-mark-1.jpg}
		}%
		\hspace{15pt}		
		\subfloat[\label{fig:butterfly_cor_gc1}]{%
		\includegraphics[scale=0.5]{images/butterfly/cor-mark-1.jpg}
		}		

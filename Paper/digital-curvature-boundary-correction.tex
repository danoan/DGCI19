...
A digital image is the result of some quantization process over an object $X$ lying in some continuous space of dimension $d$.  For example, the Gauss digitization of $X$ with grid step $h>0$ is defined as

\begin{align*}
	D_h(X) = X \cap (h\mathbb{Z})^d.
\end{align*} 
...
precise. Given any subset $Z$ of $(h\mathbb{Z})^d$, we can embed it as

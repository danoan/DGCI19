...
Generally, best results in terms of elastica energy are achieved by using a higher number of level sets. The only exception is for the digital ball, but even for this one, the error decreases as the ball radius and level sets increases. As mentioned, the computation of the estimator in different level sets decreases the number of unlabeled pixels by QPBOP. In practice, we observe that using a ball of radius $3$ is sufficient to produce good results while achieving a reasonable running time.
...
\includegraphics[scale=0.025]{images/flow-b3-ls3/Flower.eps}}%
		...
		\subfloat[\label{fig:flow_ball}]{%
		\includegraphics[scale=0.25]{images/flow-b5-ls5/InProduction.png}
		}%
		\hspace{15pt}
		\subfloat[\label{fig:flow_triangle}]{%
		\includegraphics[scale=0.25]{images/flow-b5-ls5/InProduction.png}}%
...
\includegraphics[scale=0.25]{images/flow-b5-ls5/InProduction.png}}%		
		\hspace{15pt}
		\subfloat[\label{fig:flow_flower}]{%
\includegraphics[scale=0.25]{images/flow-b10-ls10/InProduction.png}}%
...
\begin{tabular}{l|c|c|c|c}
	...
	$r=5,ls=4$ & 0.174 & 0.319 & 0.232 & 0.244\\
	$\mathbf{r=5,ls=5}$ & ? & ? & ? & ?\\	
	\hline
	$r=10,ls=2$ & ? & ? & ? & ?\\
	$r=10,ls=5$ & ? & ? & ? & ?\\

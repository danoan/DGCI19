...
\includegraphics[scale=1.0]{images/opt-regions.png}}%		
		\caption{Figure (a): White pixels denote variables labeled as one by QPBOP, while light-gray pixels variables labeled zero; Figure (b): Inverted labeling; Figure (c): Regions of interest: Background (black); Foreground (dark gray); Computation (light gray); and Optimization (white) regions.}	
	...
	\end{figure}
        ...
        digital object, which is done by removing the highlighted pixels in figure \ref{fig:naive_solution}. In different words, the solution given by the optimization method is inversed. Points with
...
Function $F$ is submodular if and only if \cite{kolmogorov04}

\begin{align*}
	F_{i,j}(0,0) + F_{i,j}(1,1) \leq F_{i,j}(0,1) + F_{i,j}(1,0)
\end{align*}
